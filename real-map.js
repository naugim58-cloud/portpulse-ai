...
function realMapPopup(i){const p=ports[i],m=window.congestionInputs&&window.congestionInputs[p[0]],score=m&&Number.isFinite(m.totalPoints)?m.totalPoints:congestionScore(i);return `<div class="leaflet-popup-title">${p[0]}</div><div class="leaflet-popup-score ${congestionTone(score)}">${score} / 100 · ${congestionStatus(score)}</div><div class="leaflet-popup-meta">평균 대기 ${p[5]}분 · 대기 선박 ${p[4]}척</div>`}
