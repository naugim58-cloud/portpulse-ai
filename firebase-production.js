...
(function(){
  ...
  Promise.all([load('https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js'),load('https://www.gstatic.com/firebasejs/10.12.2/firebase-auth-compat.js')]).then(()=>{
    ...
    portpulseFirebaseAuth.onAuthStateChanged(user=>{if(user){const email=String(user.email||'').toLowerCase();localStorage.setItem('portpulse-session',email);if(typeof window.syncAccountFavorites==='function')window.syncAccountFavorites(email);if(typeof window.render==='function')window.render()}else{localStorage.removeItem('portpulse-session');}});
